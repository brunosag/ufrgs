<script lang="ts">
	...
	import { runPreparedQuery } from '$lib/db';
	import queries from '$lib/queries.json';
	import type { Query, Result } from '$lib/types';
	...
	let {
		...
		result = $bindable()
	}: { query: Query; selectedQuery: Query | null; result: Result | null } = $props();

	let isOpen = $derived(selectedQuery?.queryId === query.queryId);

	function handleSelect(queryId: string) {
		selectedQuery =
			queryId === selectedQuery?.queryId
				? null
				: (queries.find((query) => query.queryId === queryId) as Query);
	}

	async function handleRunQuery(queryId: string, params?: Query['params']) {
		result = {
			title: selectedQuery?.title,
			description: selectedQuery?.description,
			data: await runPreparedQuery(
				queryId,
				params?.reduce(
					(acc, param) => {
